...
resource "linode_instance" "cfe-pyapp" {
    ...
    provisioner "remote-exec" {
        ...
        inline = [
            ...
            "docker run --restart always -d -p 80:8001 -e PORT=8001 --name pyapp pyapp"
        ]
    }
}
...
resource "linode_nodebalancer_node" "pycfe_nb_node_http" {
    count = var.instance_count
    nodebalancer_id = linode_nodebalancer.pycfe_nb.id
    config_id = linode_nodebalancer_config.pycfe_nb_config.id
    label = "pycfe_node_pyapp_${count.index}"
